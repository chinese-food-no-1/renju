...
public class AILevelThree : Player
{
    ...
    {
        if (
            Chessbooard.Instacne.chessStack.Count == 0)
        {
        ...
        foreach (var item in GetList(Chessbooard.Instacne.grid, (int)chessColor, true))
        {
            CreateTree(item, (int[,])Chessbooard.Instacne.grid.Clone(), 3, false);
        ...
        Chessbooard.Instacne.PlayChess(node.pos);
    }
