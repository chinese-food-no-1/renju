    ...
    public override void CheckOneLine(int[] pos, int[] offset, int chess)
    {
        ...
        while (AllNum < 7 && (!lstop || !rstop))
        {
            ...
            pos[1] + lj >= 0 && pos[1] + lj < 15 && !lstop)
                {
                    if (Chessbooard.Instacne.grid[pos[0] + li, pos[1] + lj] == chess)
                    {
                        AllNum++;
                        str = "a" + str;

                    }
                    else if (Chessbooard.Instacne.grid[pos[0] + li, pos[1] + lj] == 0)
                    {
                        AllNum++;
                        str = "_" + str;
          ...
          pos[1] + rj >= 0 && pos[1] + rj < 15 && !lfirst && !rstop)
                {
                    if (Chessbooard.Instacne.grid[pos[0] + ri, pos[1] + rj] == chess)
                    {
                        AllNum++;
                        str += "a";

                    }
                    else if (Chessbooard.Instacne.grid[pos[0] + ri, pos[1] + rj] == 0)
                    {
                        AllNum++;
                        str += "_";
